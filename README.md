...
기존 모델은 일반 문맥 상에서 가지는 의미로 임베딩 되었기 때문에 영화 장르와는 어울리지 않는 임베딩 값을 가집니다. 예를 들어 유사한 장르인 `Thriller`와 `Action`이 완전히 다른 벡터를 가지고 있는 모습을 볼 수 있습니다. 
이러한 기존 모델의 문제점을 파악하고 영화와 장르로 그래프를 구성하고 이를 기반으로 학습한 모델을 만들었습니다. 이 모델은 실제 영화와 장르의 관계가 고려되었기 때문에 유사한 장르의 벡터 값이 비슷하도록 학습되어 있습니다.
실제로 `Thriller`와 `Action`의 벡터 값이 비슷한 것을 볼 수 있습니다.
